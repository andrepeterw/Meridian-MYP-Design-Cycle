        ...
        <>
          ...
          <div className="mt-6 grid gap-6 md:grid-cols-2">
            {[processJournalContent.exemplars.product, processJournalContent.exemplars.digital].map(
              (exemplar) => (
                <div key={exemplar.label}>
                  <p className="mb-1 text-xs font-bold uppercase tracking-wide text-[var(--ink)]/50">
                    {exemplar.label}
                  </p>
                  <p className="mb-4 leading-relaxed text-[var(--ink)]/80">
                    <strong>{exemplar.projectName}</strong>
                  </p>
                  <div className="space-y-4">
                    {processJournalContent.tree.map((criterion) => (
                      ...
                      >
                        ...
                        <ul className="mt-3 space-y-3 text-sm text-[var(--ink)]/80">
                          {criterion.nodes.map((node) => (
                            <li key={node.id}>
                              <span className="font-medium text-[var(--ink)]">{node.label}</span>
                              {": "}
                              {exemplar.entries[node.id] ?? node.evidenceNote}
                            </li>
                          ))}
                        </ul>
                      </div>
